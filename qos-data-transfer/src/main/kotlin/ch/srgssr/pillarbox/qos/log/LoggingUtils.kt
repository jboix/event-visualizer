package ch.srgssr.pillarbox.qos.log
...
inline fun Logger.debug(lazyMessage: () -> String) =
  isDebugEnabled.takeIf { it }?.let { debug(lazyMessage()) }
...
inline fun Logger.info(lazyMessage: () -> String) =
  isInfoEnabled.takeIf { it }?.let { info(lazyMessage()) }
...
inline fun Logger.warn(lazyMessage: () -> String) =
  isWarnEnabled.takeIf { it }?.let { warn(lazyMessage()) }
...
inline fun Logger.error(lazyMessage: () -> String) =
  isErrorEnabled.takeIf { it }?.let { error(lazyMessage()) }
